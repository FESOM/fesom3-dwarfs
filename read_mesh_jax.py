...
r_earth=6367500.0
#test exchange:
arr = jnp.ones(partit.myDim_elem2D + partit.eDim_elem2D)
...
#ssh_rhs=test_divergence(mesh, partit, cyclic_length)

import time
print(type(partit.myDim_edge2D), type(partit.eDim_edge2D))  # Should be <class 'int'>
print(type(mesh.elem2D), type(mesh.coord_nod2D))  # Should be <class 'jaxlib.xla_extension.ArrayImpl'>


#call it first time (will be long since needs be compiled)
t1 = time.time()
ssh_rhs, minval, maxval, sumval = test_divergence(mype=partit.mype, myDim_edge2D = partit.myDim_edge2D,
